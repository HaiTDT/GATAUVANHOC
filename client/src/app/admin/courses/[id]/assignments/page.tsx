...
import { api, type Assignment, type Course } from "../../../../../lib/api";
...
export default function AdminCourseAssignmentsPage() {
  ...
  const load = async () => {
    try {
      ...
      const [courseRes, assignRes] = await Promise.all([
        ...
        api.getAdminCourseAssignments(id)
      ]);
      setCourse(courseRes);
  ...
  const handleDelete = async (assignId: string) => {
    if (!window.confirm("Xác nhận xóa bài tập này khỏi khóa học?")) return;
    try {
    ...
    <div className="max-w-5xl">
      <header className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold font-headline text-2xl text-primary">Bài tập trong Khóa học</h2>
            <p className="text-sm text-stone-500">Khóa học: {course?.title}</p>
          </div>
        </div>
        {!showAdd && (
          <button 
          ...
          >
            <span className="material-symbols-outlined text-sm">add</span> Thêm bài tập mới
          </button>
        )}
      </header>
        ...
        <div className="space-y-4">
          {assignments.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center border border-stone-200 border-dashed">
               ...
               <p className="text-stone-500">Khóa học này chưa có bài tập nào.</p>
            </div>
          ) : (
            assignments.map((a) => (
              <div key={a.id} className="bg-white rounded-2xl p-6 shadow-sm border border-stone-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-stone-500">{a.questions?.length || 0} câu hỏi • {a.points} điểm</p>
                </div>
...
// Reusing most of the logic from the previous AddAssignmentForm but pointing to createAdminCourseAssignment
function AddCourseAssignmentForm({ courseId, onCancel, onSuccess }: { courseId: string, onCancel: () => void, onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await api.createAdminCourseAssignment(courseId, form);
      onSuccess();
    } catch (err: any) {
      setError(err.message);
    } finally {
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        {form.type === 'QUIZ' && (
          <div className="pt-6 border-t border-stone-100 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-stone-900">Câu hỏi trắc nghiệm</h3>
              <button type="button" onClick={addQuestion} className="text-xs font-bold text-primary flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">add_circle</span> Thêm câu hỏi
              </button>
            </div>
            
            {form.questions.map((q, qIndex) => (
              <div key={qIndex} className="p-5 bg-stone-50 rounded-2xl border border-stone-200 relative">
                <button type="button" onClick={() => removeQuestion(qIndex)} className="absolute top-4 right-4 text-stone-400 hover:text-red-500">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <button type="button" onClick={() => updateQuestion(qIndex, { options: [...q.options, { text: "", isCorrect: false }] })} className="text-[10px] font-bold text-primary">
                      + Thêm lựa chọn
                    </button>
                  </div>
                </div>
        ...
        <div className="pt-6 border-t border-stone-100 flex justify-end gap-3">
          <button type="button" onClick={onCancel} className="px-6 py-2.5 border border-stone-300 rounded-xl text-sm font-bold text-stone-600">Hủy</button>
          <button type="submit" disabled={loading} className="px-8 py-2.5 bg-primary text-white rounded-xl text-sm font-bold shadow-lg disabled:opacity-50">
